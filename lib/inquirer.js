  ...
  inquirer.prompt([
    {
      ...
      choices: ['Télécharger des packages', 'Installation de packages téléchargés', 'Clean les packages existants', 'Quitter']
    }
  ]).then(answers => {
    const { action } = answers;

    switch (action) {
      case 'Télécharger des packages':
        inquirer.prompt([
          {
            type: 'input',
            name: 'packages',
      ...
      case 'Installation de packages téléchargés':
        installerPackages();
        break;

      case 'Clean les packages existants':
        cleanPackages();
        break;
